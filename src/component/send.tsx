...
export const createSend = function (
    setStopFunctionVisible: { (value: SetStateAction<boolean>): void },
    ...
    return [async (
        ...
        if (chatConfig?.dataProcessor?.rewritePrompt) {
            prompt = chatConfig?.dataProcessor.rewritePrompt(prompt);
        }
        const body = {
        ...
        while (true) {
            ...
            if (content) {
                const processChunk = chatConfig?.dataProcessor?.processChunk || defaultProcessChunk;
                try {
